...
def main():
    ...
    if non_event_rows:
        ...
        print("No 'Non-Events (X)' categories found in event category distribution. Skipping CSV adjustments.")

    # 4. Save Natural Disasters views percentage to report.json
    final_main_df = pd.read_csv(args.main_csv)
    final_event_df = pd.read_csv(args.event_csv)
    
    events_row = final_main_df[final_main_df['Category'] == 'Events']
    natural_disasters_row = final_event_df[final_event_df['Category'] == 'Natural Disasters']
    
    events_views_pct = 0.0
    if not events_row.empty:
        val_str = str(events_row.iloc[0]['Views %']).replace('%', '').strip()
        events_views_pct = float(val_str)
        
    natural_disasters_views_pct = 0.0
    if not natural_disasters_row.empty:
        val_str = str(natural_disasters_row.iloc[0]['Views %']).replace('%', '').strip()
        natural_disasters_views_pct = float(val_str)
        
    pct_views_natural_disasters = (events_views_pct * natural_disasters_views_pct) / 100.0
    print(f"Events views %: {events_views_pct}%")
    print(f"Natural Disasters views %: {natural_disasters_views_pct}%")
    print(f"Natural Disasters percentage of all views: {pct_views_natural_disasters:.4f}%")
    
    # Save to report.json
    from wikihistories.project_summary import load_report, write_report
    report = load_report()
    
    report["natural_disasters_views_pct"] = f"{pct_views_natural_disasters:.2f}"
    
    write_report(report)
    print("Updated report.json with Natural Disasters views percentage.")
